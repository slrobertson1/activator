        ...
        function(res,cb){
          if (!res) {
            ...
            if (!email) {
              ...
              mailer("activate",req.lang||"en_US",{code:code,authentication:code,email:email,id:id,password:user.password,request:req},from,email,attachments.activate,cb);
            }
          }
        }
      ],function (err) {
...
module.exports = {
  ...
  bulkCreateActivateNext: function (req,res,next) {
    rparam(req);
    if (req.users) {
      async.map(req.users, function(user, cb) {
        ...
        createActivate(req,function (code,message) {
          ...
          _.extend(req.activator,{code:code,message:message});
          // next();
          cb(null,{code:code,message:message});
        });
      }, function(err, results) {
        console.log(`bulkCreateActivateNext is done! err = ${err}`);
        next();
      });
    }
  },
